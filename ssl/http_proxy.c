/**
    This utility does simple (non-encrypted HTTP)
*/
...
/* accept a wellformed URL and return pointers to the host. This function modifies the URI */
...
int main(int argc, char* argv[]) {
    ...
    if (!strcmp("-p", argv[ind])) {
        if (!parse_proxy_param(argv[++indx], &proxy_host, &proxy_port, &proxy_user, &proxy_password)) {
...
int parse_proxy_param(char *proxy_spec, char **proxy_host, int *proxy_port, char **proxy_user, char **proxy_password) {
    ...
    // Techincally, the user should start the proxy spec with
    // "http://". But, be forgiving if he didn't
    if (!strcmp("http://", proxy_spec, 7)) {
        proxy_spec += 7;
    }
...
/**
* Formand and send and HTTP get command. The return value ill be 0
* on success, -1 on failure. with errno set approrpately. The caller
* must then retrieve the response.
*/ 

int http_get(int connection, const char* path, const char *host,
             const char *proxy_host, const char *post_user, const char *proxy_password) {
